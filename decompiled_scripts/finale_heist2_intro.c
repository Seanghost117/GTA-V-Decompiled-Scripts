...
void __EntryFunction__()
{
	...
	while (!bVar1)
	{
		...
		if (NETWORK::NETWORK_IS_GAME_IN_PROGRESS(SCRIPT::_GET_NAME_OF_THREAD(iVar0), "heist_ctrl_finale"))
		{
			bVar1 = true;
		}
	}
	...
	if (MISC::IS_BIT_SET(iVar0))
	{
		...
		if (ENTITY::GET_ENTITY_COORDS(bVar0, false))
		{
			...
			else
			{
				...
				{
					if (func_13(bVar0, Param0, Param1, fParam2))
					{
						bVar1 = true;
					}
				}
...
int func_4(bool bParam0, int iParam1, bool bParam2)
{
	int iVar0;
	char* sVar1;
	int iVar2;
...
int func_13(bool bParam0, struct<3> Param1, struct<3> Param2, float fParam3)
{
	struct<3> Var0;
	struct<3> Var1;
	struct<3> Var2;
