...
int func_2()
{
	...
	if (!func_3(&uVar0))
	{
		Var1 = { NETWORK::NETWORK_ARE_HANDLES_THE_SAME(bLocal_68, 0) };
		fVar2 = SYSTEM::VDIST2(Local_170 + Local_173, Var1);
		if (fVar2 < (4.5f * 4.5f))
		{
			TASK::TASK_VEHICLE_DRIVE_WANDER(bLocal_68, iLocal_70, fLocal_171, 786599);
			return 1;
		}
	}
	return 0;
...
int func_3(var uParam0)
{
	...
	if (ENTITY::GET_ENTITY_COORDS(iLocal_70, false))
	{
	}
	else
	{
	...
	if (!func_3(&uVar0))
	{
		...
		else if (PED::DETACH_SYNCHRONIZED_SCENE(Global_96285))
		{
			...
			if (!func_5(Global_98676, iLocal_70, 2))
			{
				if (bVar1 >= 0.9f)
				{
					PED::SET_PED_INTO_VEHICLE(Global_98676, iLocal_70, 2);
				}
			}
			if (!iLocal_172)
			{
				ENTITY::SET_ENTITY_COORDS(bLocal_68, 0);
				iLocal_172 = 1;
			}
			TASK::TASK_VEHICLE_TEMP_ACTION(bLocal_68, iLocal_70, 6, 50);
		}
		else
		{
			if (!func_5(Global_98676, iLocal_70, 2))
			...
			if (!func_3(&uVar0))
			{
				...
				TASK::TASK_VEHICLE_TEMP_ACTION(false, iLocal_70, 24, 50);
				...
				AUDIO::STOP_SCRIPTED_CONVERSATION(bLocal_68, iVar2);
	...
	if (!LOCALIZATION::GET_CURRENT_LANGUAGE())
	{
		...
		if (MISC::IS_BIT_SET(iParam1))
		{
			...
			if (fVar1 > 250f)
			{
				...
				{
					if (!ENTITY::IS_ENTITY_ON_SCREEN(bParam1))
					{
						return 0;
					}
				}
		...
		else if (bParam1 == func_9(Global_98676) && bParam1 != 0)
		{
			Global_98676 = 0;
		}
	}
...
void func_22()
{
	...
	iVar0 = 0;
	while ((!MISC::IS_BIT_SET(Global_98676) || !MISC::IS_BIT_SET(iLocal_70)) || (!ENTITY::DOES_ENTITY_EXIST(iLocal_69) && iVar0 < 400))
	{
		if (!MISC::IS_BIT_SET(iLocal_70))
		{
			iLocal_70 = VEHICLE::GET_RANDOM_VEHICLE_IN_SPHERE(Local_173 + Local_169, 15f, iLocal_71, 2);
		}
		iVar0++;
	...
	if (!PED::IS_PED_IN_ANY_POLICE_VEHICLE(iLocal_70, 0))
	{
		VEHICLE::SET_VEHICLE_ENGINE_ON(bLocal_70, true, true, false);
		VEHICLE::SET_VEHICLE_DOORS_LOCKED(bLocal_70, 2);
		VEHICLE::SET_VEHICLE_ON_GROUND_PROPERLY(bLocal_70, 5f);
		bLocal_68 = PED::CREATE_PED_INSIDE_VEHICLE(bLocal_70, 4, iLocal_69, -1, true, true);
		PED::SET_PED_RANDOM_COMPONENT_VARIATION(bLocal_68, 0);
...
void func_50()
{
	if (!PED::IS_PED_IN_ANY_POLICE_VEHICLE(bLocal_68, 0) && ENTITY::DOES_ENTITY_BELONG_TO_THIS_SCRIPT(bLocal_68, true))
	{
		ENTITY::SET_PED_AS_NO_LONGER_NEEDED(&bLocal_68);
	}
	if (!PED::IS_PED_IN_ANY_POLICE_VEHICLE(bLocal_70, 0) && ENTITY::DOES_ENTITY_BELONG_TO_THIS_SCRIPT(bLocal_70, true))
	{
