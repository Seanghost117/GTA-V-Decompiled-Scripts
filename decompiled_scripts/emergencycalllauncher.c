...
void func_3()
{
	if (func_6())
	{
		...
		if (INTERIOR::GET_INTERIOR_AT_COORDS(AUDIO::SET_AMBIENT_ZONE_STATE_PERSISTENT(&uLocal_210)) == 0)
		{
			func_4();
		}
		else
...
void func_7()
{
	switch (iLocal_44)
	{
		...
		case 1:
			if (INTERIOR::GET_INTERIOR_AT_COORDS(joaat("emergencycall")) == 0)
			{
				if (!func_35(130))
				{
					iLocal_43 = 1;
...
{
	Global_31192 = 1;
	StringCopy(&Global_31193, TASK::IS_PED_BEING_ARRESTED(), 32);
}
