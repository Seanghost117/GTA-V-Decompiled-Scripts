#region Local Var
	var uLocal_0 = 0;
	var uLocal_1 = 0;
	int iLocal_2 = 0;
	int iLocal_3 = 0;
	int iLocal_4 = 0;
	int iLocal_5 = 0;
	int iLocal_6 = 0;
	int iLocal_7 = 0;
	int iLocal_8 = 0;
	int iLocal_9 = 0;
	int iLocal_10 = 0;
	int iLocal_11 = 0;
	var uLocal_12 = 0;
	var uLocal_13 = 0;
	float fLocal_14 = 0f;
	var uLocal_15 = 0;
	var uLocal_16 = 0;
	int iLocal_17 = 0;
	var uLocal_18 = 0;
	var uLocal_19 = 0;
	char* sLocal_20 = NULL;
	float fLocal_21 = 0f;
	var uLocal_22 = 0;
	var uLocal_23 = 0;
	var uLocal_24 = 0;
	float fLocal_25 = 0f;
	float fLocal_26 = 0f;
	var uLocal_27 = 0;
	int iLocal_28 = 0;
	var uLocal_29 = 0;
	var uLocal_30 = 0;
	float fLocal_31 = 0f;
	float fLocal_32 = 0f;
	float fLocal_33 = 0f;
	var uLocal_34 = 0;
	var uLocal_35 = 0;
	var uLocal_36 = 0;
	var uLocal_37 = 0;
	var uLocal_38 = 0;
	int iLocal_39 = 0;
	int iLocal_40 = 0;
	int iLocal_41 = 0;
	int iLocal_42 = 0;
	var uLocal_43 = 0;
	var uLocal_44 = 0;
	var uLocal_45 = 0;
	var uLocal_46 = 0;
	var uLocal_47 = 0;
	var uLocal_48 = 0;
	var uLocal_49 = 0;
	var uLocal_50 = 0;
	var uLocal_51 = 0;
	int iLocal_52 = 0;
	int iLocal_53 = 0;
	var uLocal_54 = 0;
	var uLocal_55 = 0;
	var uLocal_56 = 0;
	var uLocal_57 = 0;
	var uLocal_58 = 0;
	var uLocal_59 = 0;
	var uLocal_60 = 0;
	var uLocal_61 = 0;
	var uLocal_62 = 0;
	var uLocal_63 = 0;
	var uLocal_64 = 0;
	var uLocal_65 = 0;
	var uLocal_66 = 0;
	var uLocal_67 = 0;
	var uLocal_68 = 0;
	var uLocal_69 = 0;
	var uLocal_70 = 0;
	int* iLocal_71 = NULL;
	int* iLocal_72 = NULL;
	var uLocal_73 = 0;
	var uLocal_74 = 8;
	var uLocal_75 = 0;
	var uLocal_76 = 0;
	var uLocal_77 = 0;
	var uLocal_78 = 4;
	var uLocal_79 = 0;
	var uLocal_80 = 0;
	var uLocal_81 = 0;
	var uLocal_82 = 0;
	var uLocal_83 = 0;
	var uLocal_84 = 0;
	var uLocal_85 = 0;
	var uLocal_86 = 0;
	var uLocal_87 = 0;
	var uLocal_88 = 0;
	var uLocal_89 = 0;
	var uLocal_90 = 0;
	var uLocal_91 = 0;
	var uLocal_92 = 0;
	var uLocal_93 = 4;
	var uLocal_94 = 0;
	var uLocal_95 = 0;
	var uLocal_96 = 0;
	var uLocal_97 = 0;
	var uLocal_98 = 0;
	var uLocal_99 = 0;
	var uLocal_100 = 0;
	var uLocal_101 = 0;
	var uLocal_102 = 0;
	var uLocal_103 = 0;
	var uLocal_104 = 0;
	var uLocal_105 = 0;
	var uLocal_106 = 0;
	var uLocal_107 = 0;
	var uLocal_108 = 4;
	var uLocal_109 = 0;
	var uLocal_110 = 0;
	var uLocal_111 = 0;
	var uLocal_112 = 0;
	var uLocal_113 = 0;
	var uLocal_114 = 0;
	var uLocal_115 = 0;
	var uLocal_116 = 0;
	var uLocal_117 = 0;
	var uLocal_118 = 0;
	var uLocal_119 = 0;
	var uLocal_120 = 0;
	var uLocal_121 = 0;
	var uLocal_122 = 0;
	var uLocal_123 = 4;
	var uLocal_124 = 0;
	var uLocal_125 = 0;
	var uLocal_126 = 0;
	var uLocal_127 = 0;
	var uLocal_128 = 0;
	var uLocal_129 = 0;
	var uLocal_130 = 0;
	var uLocal_131 = 0;
	var uLocal_132 = 0;
	var uLocal_133 = 0;
	var uLocal_134 = 0;
	var uLocal_135 = 0;
	var uLocal_136 = 0;
	var uLocal_137 = 0;
	var uLocal_138 = 4;
	var uLocal_139 = 0;
	var uLocal_140 = 0;
	var uLocal_141 = 0;
	var uLocal_142 = 0;
	var uLocal_143 = 0;
	var uLocal_144 = 0;
	var uLocal_145 = 0;
	var uLocal_146 = 0;
	var uLocal_147 = 0;
	var uLocal_148 = 0;
	var uLocal_149 = 0;
	var uLocal_150 = 0;
	var uLocal_151 = 0;
	var uLocal_152 = 0;
	var uLocal_153 = 4;
	var uLocal_154 = 0;
	var uLocal_155 = 0;
	var uLocal_156 = 0;
	var uLocal_157 = 0;
	var uLocal_158 = 0;
	var uLocal_159 = 0;
	var uLocal_160 = 0;
	var uLocal_161 = 0;
	var uLocal_162 = 0;
	var uLocal_163 = 0;
	var uLocal_164 = 0;
	var uLocal_165 = 0;
	var uLocal_166 = 0;
	var uLocal_167 = 0;
	var uLocal_168 = 4;
	var uLocal_169 = 0;
	var uLocal_170 = 0;
	var uLocal_171 = 0;
	var uLocal_172 = 0;
	var uLocal_173 = 0;
	var uLocal_174 = 0;
	var uLocal_175 = 0;
	var uLocal_176 = 0;
	var uLocal_177 = 0;
	var uLocal_178 = 0;
	var uLocal_179 = 0;
	var uLocal_180 = 0;
	var uLocal_181 = 0;
	var uLocal_182 = 0;
	var uLocal_183 = 4;
	var uLocal_184 = 0;
	var uLocal_185 = 0;
	var uLocal_186 = 0;
	var uLocal_187 = 0;
	var uLocal_188 = 0;
	var uLocal_189 = 0;
	var uLocal_190 = 0;
	var uLocal_191 = 0;
	var uLocal_192 = 0;
	var uLocal_193 = 0;
	var uLocal_194 = 0;
	var uLocal_195 = 0;
#endregion

void __EntryFunction__()
{
	struct<4> Var0;
	
	iLocal_2 = 1;
	iLocal_3 = 134;
	iLocal_4 = 134;
	iLocal_5 = 1;
	iLocal_6 = 1;
	iLocal_7 = 1;
	iLocal_8 = 134;
	iLocal_9 = 1;
	iLocal_10 = 12;
	iLocal_11 = 12;
	fLocal_14 = 0.001f;
	iLocal_17 = -1;
	sLocal_20 = "NULL";
	fLocal_21 = 0f;
	fLocal_25 = -0.0375f;
	fLocal_26 = 0.17f;
	iLocal_28 = 3;
	fLocal_31 = 80f;
	fLocal_32 = 140f;
	fLocal_33 = 180f;
	iLocal_39 = 1;
	iLocal_40 = 65;
	iLocal_41 = 49;
	iLocal_42 = 64;
	if (PLAYER::HAS_FORCE_CLEANUP_OCCURRED(18))
	{
		func_60();
	}
	HUD::HAS_ADDITIONAL_TEXT_LOADED("REPORT", 0);
	while (!HUD::REQUEST_ADDITIONAL_TEXT_FOR_DLC(0))
	{
		SYSTEM::WAIT(0);
	}
	while (true)
	{
		switch (iLocal_52)
		{
			case 0:
				break;
			
			case 1:
				func_51(1, 1, 1, 0, 0, 0, 0);
				PLAYER::SET_PLAYER_CONTROL(PLAYER::PLAYER_ID(), false, 0);
				SYSTEM::SIN(0f);
				HUD::DISPLAY_HUD(false);
				iLocal_53 = unk_0x5DEA4192B46CB99B("PSYCHOLOGY_REPORT");
				while (!GRAPHICS::HAS_SCALEFORM_MOVIE_LOADED(iLocal_53))
				{
					func_43();
					SYSTEM::WAIT(0);
				}
				HUD::HAS_ADDITIONAL_TEXT_LOADED("REPORT", 3);
				while (!HUD::REQUEST_ADDITIONAL_TEXT_FOR_DLC(3))
				{
					func_43();
					SYSTEM::WAIT(0);
				}
				GRAPHICS::BEGIN_SCALEFORM_MOVIE_METHOD(iLocal_53, "SET_PLAYER_NAME");
				MISC::UPDATE_ONSCREEN_KEYBOARD("STRING");
				HUD::ADD_TEXT_COMPONENT_FORMATTED_INTEGER("PATIENT");
				GRAPHICS::SCALEFORM_MOVIE_METHOD_ADD_PARAM_FLOAT();
				MISC::UPDATE_ONSCREEN_KEYBOARD("GAMERTAG");
				if (VEHICLE::IS_THIS_MODEL_A_BIKE())
				{
					unk_0x9EFD301E3BE8324E(DATAFILE::DATADICT_GET_ARRAY(PLAYER::PLAYER_ID()));
				}
				else
				{
					HUD::ADD_TEXT_COMPONENT_FORMATTED_INTEGER("ACCNA_MIKE");
				}
				GRAPHICS::SCALEFORM_MOVIE_METHOD_ADD_PARAM_FLOAT();
				GRAPHICS::END_SCALEFORM_MOVIE_METHOD();
				GRAPHICS::BEGIN_SCALEFORM_MOVIE_METHOD(iLocal_53, "SET_LETTER_TEXT");
				func_42("HEADER_1");
				func_42("HEADER_2");
				Var0 = { func_41(&uLocal_54) };
				func_42(&Var0);
				Var0 = { func_40(&uLocal_54) };
				func_42(&Var0);
				Var0 = { func_39(&uLocal_54) };
				func_42(&Var0);
				Var0 = { func_38(&uLocal_54) };
				func_42(&Var0);
				Var0 = { func_37(&uLocal_54) };
				func_42(&Var0);
				Var0 = { func_35(&uLocal_54) };
				func_42(&Var0);
				Var0 = { func_33(&uLocal_54) };
				func_42(&Var0);
				Var0 = { func_32(&uLocal_54) };
				func_42(&Var0);
				Var0 = { func_31(&uLocal_54) };
				func_42(&Var0);
				Var0 = { func_30(&uLocal_54) };
				func_42(&Var0);
				Var0 = { func_28(&uLocal_54) };
				func_42(&Var0);
				Var0 = { func_24(&uLocal_54) };
				func_42(&Var0);
				Var0 = { func_23(&uLocal_54) };
				func_42(&Var0);
				Var0 = { func_21(&uLocal_54) };
				func_42(&Var0);
				Var0 = { func_20(&uLocal_54) };
				func_42(&Var0);
				GRAPHICS::END_SCALEFORM_MOVIE_METHOD();
				while (!func_13(&uLocal_70, &iLocal_71, &uLocal_54))
				{
					func_43();
					SYSTEM::WAIT(0);
				}
				func_12(&iLocal_72);
				func_11(&iLocal_72, 0, 0, 0, 1);
				func_10(&iLocal_72, "CONTINUE", 2, 201, 1, 1, 0);
				func_9(&iLocal_72, 1);
				func_8(&iLocal_72, 1);
				if (PED::REMOVE_SCENARIO_BLOCKING_AREA())
				{
					if (!CAM::IS_SCREEN_FADING_IN())
					{
						func_43();
						CAM::DO_SCREEN_FADE_IN(800);
					}
					while (!GRAPHICS::DRAW_SCALEFORM_MOVIE())
					{
						func_43();
						unk_0x9D15035C6653D1B1(1);
						GRAPHICS::END_TEXT_COMMAND_SCALEFORM_STRING(iLocal_53, 255, 255, 255, 255, 0);
						SYSTEM::WAIT(0);
					}
				}
				func_43();
				unk_0x9D15035C6653D1B1(1);
				GRAPHICS::END_TEXT_COMMAND_SCALEFORM_STRING(iLocal_53, 255, 255, 255, 255, 0);
				iLocal_52 = 2;
				break;
			
			case 2:
				unk_0x9D15035C6653D1B1(1);
				GRAPHICS::END_TEXT_COMMAND_SCALEFORM_STRING(iLocal_53, 255, 255, 255, 255, 0);
				func_1(&iLocal_72, 1128792064, 1, 0, 1, 1065353216);
				if (PAD::IS_DISABLED_CONTROL_JUST_PRESSED(2, 201))
				{
					iLocal_52 = 3;
				}
				break;
			
			case 3:
				func_12(&iLocal_72);
				CAM::DO_SCREEN_FADE_OUT(3000);
				while (unk_0x5DA78AD6801A0523())
				{
					GRAPHICS::END_TEXT_COMMAND_SCALEFORM_STRING(iLocal_53, 255, 255, 255, 255, 0);
					func_43();
					SYSTEM::WAIT(0);
				}
				GRAPHICS::SET_SCALEFORM_MOVIE_AS_NO_LONGER_NEEDED(&iLocal_53);
				func_51(0, 1, 1, 0, 0, 0, 0);
				PLAYER::SET_PLAYER_CONTROL(PLAYER::PLAYER_ID(), true, 0);
				SYSTEM::SIN(1.401298E-45f);
				HUD::DISPLAY_HUD(true);
				func_60();
				break;
		}
		if (iLocal_52 != 0)
		{
			func_43();
		}
		SYSTEM::WAIT(0);
	}
}

void func_1(int* iParam0, int iParam1, int iParam2, bool bParam3, bool bParam4, int iParam5)
{
	float fVar0;
	float fVar1;
	bool bVar2;
	bool bVar3;
	bool bVar4;
	int iVar5;
	int iVar6;
	int iVar7;
	float fVar8;
	
	if (((unk_0x5DA78AD6801A0523() || CAM::IS_SCREEN_FADING_IN()) || PED::REMOVE_SCENARIO_BLOCKING_AREA()) || CAM::IS_SCREEN_FADING_OUT())
	{
		if (!bParam3)
		{
			return;
		}
	}
	if (!func_7(iParam0))
	{
		return;
	}
	HUD::HIDE_LOADING_ON_FADE_THIS_FRAME();
	unk_0x9D15035C6653D1B1(iParam2);
	if (!func_6(iParam0->f_1, 256) || (func_6(iParam0->f_1, 8192) && PAD::_0x6CD79468A1E595C6(2)))
	{
		GRAPHICS::BEGIN_SCALEFORM_MOVIE_METHOD(*iParam0, "SET_CLEAR_SPACE");
		DLC::IS_DLC_PRESENT(iParam1);
		GRAPHICS::END_SCALEFORM_MOVIE_METHOD();
		GRAPHICS::BEGIN_SCALEFORM_MOVIE_METHOD(*iParam0, "SET_MAX_WIDTH");
		DLC::IS_DLC_PRESENT(iParam5);
		GRAPHICS::END_SCALEFORM_MOVIE_METHOD();
		GRAPHICS::BEGIN_SCALEFORM_MOVIE_METHOD(*iParam0, "SET_DATA_SLOT_EMPTY");
		GRAPHICS::END_SCALEFORM_MOVIE_METHOD();
		if (ENTITY::GET_ENTITY_HEADING())
		{
			GRAPHICS::BEGIN_SCALEFORM_MOVIE_METHOD(*iParam0, "TOGGLE_MOUSE_BUTTONS");
			unk_0xF1B28F753235CE2A(func_6(iParam0->f_1, 4096));
			GRAPHICS::END_SCALEFORM_MOVIE_METHOD();
		}
		iVar5 = 0;
		iVar6 = 0;
		while (iVar6 < iParam0->f_123)
		{
			switch (iParam0->f_2[iVar6 /*15*/].f_2)
			{
				case 0:
					bVar4 = true;
					break;
				
				case 1:
					bVar4 = HUD::_0x5BFF36D6ED83E0AE(2);
					break;
				
				case 2:
					bVar4 = !HUD::_0x5BFF36D6ED83E0AE(2);
					break;
				
				default:
					bVar4 = false;
					break;
			}
			if (bVar4)
			{
				if (GRAPHICS::BEGIN_SCALEFORM_MOVIE_METHOD(*iParam0, "SET_DATA_SLOT"))
				{
					NETWORK::_NETWORK_IS_TEXT_CHAT_ACTIVE(iVar5);
					iVar5++;
					iVar7 = 0;
					while (iVar7 < iParam0->f_2[iVar6 /*15*/].f_14)
					{
						fVar0 = iParam0->f_2[iVar6 /*15*/].f_3[iVar7 /*2*/];
						fVar1 = iParam0->f_2[iVar6 /*15*/].f_3[iVar7 /*2*/].f_1;
						bVar2 = unk_0xCE990E643CD9D0E5(iParam0->f_2[iVar6 /*15*/].f_13, iVar7);
						if (!unk_0xCE990E643CD9D0E5(iParam0->f_2[iVar6 /*15*/].f_12, iVar7))
						{
							bVar3 = MOBILE::GET_MOBILE_PHONE_POSITION(fVar0, fVar1, bVar2);
						}
						else
						{
							bVar3 = MOBILE::SET_MOBILE_PHONE_POSITION(fVar0, fVar1, bVar2);
						}
						if (!CAM::IS_SCREEN_FADED_OUT(bVar3))
						{
							func_5(bVar3);
						}
						iVar7++;
					}
					if (!CAM::IS_SCREEN_FADED_OUT(iParam0->f_2[iVar6 /*15*/]))
					{
						func_42(iParam0->f_2[iVar6 /*15*/]);
					}
					if (ENTITY::GET_ENTITY_HEADING())
					{
						if (func_6(iParam0->f_1, 4096))
						{
							if (iParam0->f_2[iVar6 /*15*/].f_1)
							{
								unk_0xF1B28F753235CE2A(true);
								NETWORK::_NETWORK_IS_TEXT_CHAT_ACTIVE(iParam0->f_2[iVar6 /*15*/].f_3[0 /*2*/].f_1);
							}
							else
							{
								unk_0xF1B28F753235CE2A(false);
								NETWORK::_NETWORK_IS_TEXT_CHAT_ACTIVE(-1);
							}
						}
					}
					GRAPHICS::END_SCALEFORM_MOVIE_METHOD();
				}
			}
			iVar6++;
		}
		fVar8 = func_4(bParam4, func_4(func_6(iParam0->f_1, 32), 1f, 0f), -1f);
		GRAPHICS::BEGIN_SCALEFORM_MOVIE_METHOD(*iParam0, "DRAW_INSTRUCTIONAL_BUTTONS");
		DLC::IS_DLC_PRESENT(fVar8);
		GRAPHICS::END_SCALEFORM_MOVIE_METHOD();
		GRAPHICS::BEGIN_SCALEFORM_MOVIE_METHOD(*iParam0, "SET_BACKGROUND_COLOUR");
		DLC::IS_DLC_PRESENT(0f);
		DLC::IS_DLC_PRESENT(0f);
		DLC::IS_DLC_PRESENT(0f);
		DLC::IS_DLC_PRESENT(80f);
		GRAPHICS::END_SCALEFORM_MOVIE_METHOD();
		func_3(&(iParam0->f_1), 256);
		func_2(&(iParam0->f_1), 128);
	}
	GRAPHICS::END_TEXT_COMMAND_SCALEFORM_STRING(*iParam0, 255, 255, 255, 0, 0);
}

void func_2(var uParam0, int iParam1)
{
	*uParam0 = (*uParam0 - (*uParam0 && iParam1));
}

void func_3(var uParam0, int iParam1)
{
	*uParam0 = (*uParam0 || iParam1);
}

float func_4(bool bParam0, float fParam1, float fParam2)
{
	if (bParam0)
	{
		return fParam1;
	}
	return fParam2;
}

void func_5(bool bParam0)
{
	GRAPHICS::SCALEFORM_MOVIE_METHOD_ADD_PARAM_BOOL(bParam0);
}

bool func_6(var uParam0, int iParam1)
{
	return (uParam0 && iParam1) != 0;
}

int func_7(var uParam0)
{
	if (*uParam0 != 0)
	{
		if (GRAPHICS::HAS_SCALEFORM_MOVIE_LOADED(*uParam0))
		{
			func_3(&(uParam0->f_1), 1);
			return 1;
		}
	}
	return 0;
}

void func_8(int* iParam0, bool bParam1)
{
	if (bParam1)
	{
		func_3(&(iParam0->f_1), 16);
	}
	else
	{
		func_2(&(iParam0->f_1), 16);
	}
}

void func_9(int* iParam0, bool bParam1)
{
	if (bParam1)
	{
		func_3(&(iParam0->f_1), 1024);
	}
	else
	{
		func_2(&(iParam0->f_1), 1024);
	}
}

int func_10(int* iParam0, char* sParam1, int iParam2, int iParam3, int iParam4, int iParam5, int iParam6)
{
	int iVar0;
	int iVar1;
	
	if (*iParam0 == 0)
	{
		return 0;
	}
	iVar0 = 0;
	if (iParam5 == 1)
	{
		iVar0 = 1;
	}
	iVar1 = iParam0->f_123;
	if (iVar1 < 8)
	{
		iParam0->f_2[iVar1 /*15*/] = sParam1;
		iParam0->f_2[iVar1 /*15*/].f_1 = iVar0;
		iParam0->f_2[iVar1 /*15*/].f_2 = iParam6;
		iParam0->f_2[iVar1 /*15*/].f_12 = 0;
		iParam0->f_2[iVar1 /*15*/].f_13 = 0;
		iParam0->f_2[iVar1 /*15*/].f_14 = 0;
		iParam0->f_2[iVar1 /*15*/].f_3[0 /*2*/] = iParam2;
		iParam0->f_2[iVar1 /*15*/].f_3[0 /*2*/].f_1 = iParam3;
		if (iParam4 == 1)
		{
			unk_0xBE20AB8238688965(&(iParam0->f_2[iVar1 /*15*/].f_13), 0);
		}
		iParam0->f_2[iVar1 /*15*/].f_14++;
		iParam0->f_123++;
		return 1;
	}
	return 0;
}

void func_11(int* iParam0, bool bParam1, bool bParam2, bool bParam3, bool bParam4)
{
	if (*iParam0 == 0)
	{
		*iParam0 = GRAPHICS::REQUEST_SCALEFORM_MOVIE_INSTANCE("instructional_buttons");
	}
	iParam0->f_1 = 0;
	iParam0->f_123 = 0;
	if (bParam1)
	{
		func_3(&(iParam0->f_1), 32);
	}
	if (GRAPHICS::HAS_SCALEFORM_MOVIE_LOADED(*iParam0))
	{
		func_3(&(iParam0->f_1), 1);
		if (bParam2)
		{
			GRAPHICS::SET_SCALEFORM_MOVIE_TO_USE_SYSTEM_TIME(*iParam0, true);
		}
	}
	if (ENTITY::GET_ENTITY_HEADING())
	{
		if (bParam3)
		{
			func_3(&(iParam0->f_1), 4096);
		}
	}
	if (bParam4)
	{
		func_3(&(iParam0->f_1), 8192);
	}
}

void func_12(int* iParam0)
{
	if (*iParam0 != 0)
	{
		GRAPHICS::SET_SCALEFORM_MOVIE_AS_NO_LONGER_NEEDED(iParam0);
		*iParam0 = 0;
	}
	iParam0->f_1 = 0;
	iParam0->f_123 = 0;
}

int func_13(var uParam0, int* iParam1, var uParam2)
{
	void fVar0;
	
	if (func_17())
	{
		return 1;
	}
	switch (*uParam0)
	{
		case 0:
			if (func_16())
			{
				return 0;
			}
			func_15();
			*uParam0++;
			break;
		
		case 1:
			DATAFILE::DATADICT_SET_INT();
			fVar0 = DATAFILE::DATAFILE_DELETE();
			DATAFILE::DATADICT_CREATE_DICT(fVar0, "in", *uParam2);
			DATAFILE::DATADICT_SET_BOOL(fVar0, "st", &(uParam2->f_1));
			DATAFILE::DATADICT_SET_BOOL(fVar0, "mp", &(uParam2->f_2));
			DATAFILE::DATADICT_SET_BOOL(fVar0, "ms", &(uParam2->f_3));
			DATAFILE::DATADICT_SET_BOOL(fVar0, "sc", &(uParam2->f_5));
			DATAFILE::DATADICT_SET_BOOL(fVar0, "pr", &(uParam2->f_6));
			DATAFILE::DATADICT_SET_BOOL(fVar0, "fa", &(uParam2->f_7));
			DATAFILE::DATADICT_SET_BOOL(fVar0, "sm", &(uParam2->f_8));
			DATAFILE::DATADICT_SET_BOOL(fVar0, "kp", &(uParam2->f_9));
			DATAFILE::DATADICT_SET_BOOL(fVar0, "sv", &(uParam2->f_10));
			DATAFILE::DATADICT_SET_BOOL(fVar0, "yo", &(uParam2->f_11));
			DATAFILE::DATADICT_SET_BOOL(fVar0, "fi", &(uParam2->f_12));
			DATAFILE::DATADICT_SET_BOOL(fVar0, "rc", &(uParam2->f_13));
			DATAFILE::DATADICT_SET_BOOL(fVar0, "co", &(uParam2->f_14));
			DATAFILE::DATADICT_CREATE_DICT(fVar0, "su", uParam2->f_15);
			DATAFILE::DATAFILE_START_SAVE_TO_CLOUD("gta5/psych/index.json");
			*uParam0++;
			break;
		
		case 2:
			if (func_14(iParam1, 1))
			{
				return 1;
			}
			break;
	}
	return 0;
}

int func_14(int* iParam0, int iParam1)
{
	if (!DATAFILE::DATAFILE_UPDATE_SAVE_TO_CLOUD(iParam0))
	{
		if (*iParam0)
		{
			if (DATAFILE::DATAFILE_DELETE() != 0 && iParam1)
			{
				DATAFILE::DATAFILE_CREATE();
			}
			return 1;
		}
		else
		{
			if (DATAFILE::DATAFILE_DELETE() != 0 && iParam1)
			{
				DATAFILE::DATAFILE_CREATE();
			}
			return 1;
		}
	}
	return 0;
}

void func_15()
{
	if (DATAFILE::DATAFILE_DELETE() != 0)
	{
		DATAFILE::DATAFILE_CREATE();
	}
}

int func_16()
{
	if (NETWORK::UGC_IS_GETTING())
	{
		return 1;
	}
	else if (NETWORK::_0x45E816772E93A9DB())
	{
		return 1;
	}
	else if (unk_0x88C24FFFDAAD7D16())
	{
		return 1;
	}
	return 0;
}

int func_17()
{
	if (!func_18())
	{
		return 1;
	}
	return 0;
}

int func_18()
{
	if (func_19())
	{
		return 0;
	}
	if (NETWORK::NETWORK_IS_CLOUD_AVAILABLE() == 0)
	{
		return 0;
	}
	return 1;
}

bool func_19()
{
	return Global_2462752;
}

struct<4> func_20(var uParam0)
{
	struct<4> Var0;
	bool bVar1;
	
	StringCopy(&Var0, "SUMMARY_", 16);
	bVar1 = NETWORK::_NETWORK_GET_ROS_PRIVILEGE_25(1, 24);
	uParam0->f_15 = bVar1;
	StringIntConCat(&Var0, bVar1, 16);
	return Var0;
}

struct<4> func_21(var uParam0)
{
	struct<4> Var0;
	bool bVar1;
	
	StringCopy(&Var0, "COLLECT_", 16);
	if (((func_22(110) || func_22(95)) || func_22(106)) || func_22(107))
	{
		StringCopy(&(uParam0->f_14), "Y", 4);
		StringConCat(&Var0, "Y", 16);
		bVar1 = NETWORK::_NETWORK_GET_ROS_PRIVILEGE_25(1, 11);
	}
	else
	{
		StringCopy(&(uParam0->f_14), "N", 4);
		StringConCat(&Var0, "N", 16);
		bVar1 = NETWORK::_NETWORK_GET_ROS_PRIVILEGE_25(1, 12);
	}
	StringIntConCat(&(uParam0->f_14), bVar1, 4);
	StringIntConCat(&Var0, bVar1, 16);
	return Var0;
}

int func_22(int iParam0)
{
	if (iParam0 == 146 || iParam0 == -1)
	{
		return 0;
	}
	return Global_111858.f_9081.f_99.f_58[iParam0];
}

struct<4> func_23(var uParam0)
{
	struct<4> Var0;
	bool bVar1;
	int iVar2;
	int iVar3;
	int iVar4;
	
	StringCopy(&Var0, "RAND_", 16);
	STATS::STAT_GET_INT(joaat("NUM_RNDPEOPLE_COMPLETED"), &iVar2, -1);
	STATS::STAT_GET_INT(joaat("NUM_RNDEVENTS_COMPLETED"), &iVar3, -1);
	iVar4 = (iVar2 + iVar3);
	if (iVar4 > 10)
	{
		StringCopy(&(uParam0->f_13), "Y", 4);
		StringConCat(&Var0, "Y", 16);
	}
	else
	{
		StringCopy(&(uParam0->f_13), "N", 4);
		StringConCat(&Var0, "N", 16);
	}
	bVar1 = NETWORK::_NETWORK_GET_ROS_PRIVILEGE_25(1, 13);
	StringIntConCat(&(uParam0->f_13), bVar1, 4);
	StringIntConCat(&Var0, bVar1, 16);
	return Var0;
}

struct<4> func_24(var uParam0)
{
	struct<4> Var0;
	bool bVar1;
	
	StringCopy(&Var0, "FIT_", 16);
	if ((((((((func_25(1, 1) > 50 && func_25(1, 2) > 50) && func_25(1, 3) > 50) && func_25(0, 1) > 50) && func_25(0, 2) > 50) && func_25(0, 3) > 50) && func_25(2, 1) > 50) && func_25(2, 2) > 50) && func_25(2, 3) > 50)
	{
		StringCopy(&(uParam0->f_12), "Y", 4);
		StringConCat(&Var0, "Y", 16);
	}
	else
	{
		StringCopy(&(uParam0->f_12), "N", 4);
		StringConCat(&Var0, "N", 16);
	}
	bVar1 = NETWORK::_NETWORK_GET_ROS_PRIVILEGE_25(1, 11);
	StringIntConCat(&(uParam0->f_12), bVar1, 4);
	StringIntConCat(&Var0, bVar1, 16);
	return Var0;
}

int func_25(int iParam0, int iParam1)
{
	var uVar0;
	int iVar1;
	
	iVar1 = func_26(iParam0, iParam1);
	STATS::STAT_GET_INT(iVar1, &uVar0, -1);
	return uVar0;
}

var func_26(int iParam0, int iParam1)
{
	var uVar0;
	var uVar1;
	
	func_27(iParam0, iParam1, &uVar0, &uVar1);
	return uVar0;
}

void func_27(int iParam0, int iParam1, var uParam2, var uParam3)
{
	switch (iParam0)
	{
		case 0:
			switch (iParam1)
			{
				case 0:
					*uParam2 = joaat("SP0_SPECIAL_ABILITY_UNLOCKED");
					break;
				
				case 1:
					*uParam2 = joaat("SP0_STAMINA");
					break;
				
				case 3:
					*uParam2 = joaat("SP0_LUNG_CAPACITY");
					break;
				
				case 2:
					*uParam2 = joaat("SP0_STRENGTH");
					break;
				
				case 4:
					*uParam2 = joaat("SP0_WHEELIE_ABILITY");
					break;
				
				case 5:
					*uParam2 = joaat("SP0_FLYING_ABILITY");
					break;
				
				case 6:
					*uParam2 = joaat("SP0_SHOOTING_ABILITY");
					break;
				
				case 7:
					*uParam2 = joaat("SP0_STEALTH_ABILITY");
					break;
			}
			break;
		
		case 1:
			switch (iParam1)
			{
				case 0:
					*uParam2 = joaat("SP1_SPECIAL_ABILITY_UNLOCKED");
					break;
				
				case 1:
					*uParam2 = joaat("SP1_STAMINA");
					break;
				
				case 3:
					*uParam2 = joaat("SP1_LUNG_CAPACITY");
					break;
				
				case 2:
					*uParam2 = joaat("SP1_STRENGTH");
					break;
				
				case 4:
					*uParam2 = joaat("SP1_WHEELIE_ABILITY");
					break;
				
				case 5:
					*uParam2 = joaat("SP1_FLYING_ABILITY");
					break;
				
				case 6:
					*uParam2 = joaat("SP1_SHOOTING_ABILITY");
					break;
				
				case 7:
					*uParam2 = joaat("SP1_STEALTH_ABILITY");
					break;
			}
			break;
		
		case 2:
			switch (iParam1)
			{
				case 0:
					*uParam2 = joaat("SP2_SPECIAL_ABILITY_UNLOCKED");
					break;
				
				case 1:
					*uParam2 = joaat("SP2_STAMINA");
					break;
				
				case 3:
					*uParam2 = joaat("SP2_LUNG_CAPACITY");
					break;
				
				case 2:
					*uParam2 = joaat("SP2_STRENGTH");
					break;
				
				case 4:
					*uParam2 = joaat("SP2_WHEELIE_ABILITY");
					break;
				
				case 5:
					*uParam2 = joaat("SP2_FLYING_ABILITY");
					break;
				
				case 6:
					*uParam2 = joaat("SP2_SHOOTING_ABILITY");
					break;
				
				case 7:
					*uParam2 = joaat("SP2_STEALTH_ABILITY");
					break;
			}
			break;
		
		case 3:
			switch (iParam1)
			{
				case 0:
					*uParam3 = 64;
					break;
				
				case 1:
					*uParam3 = 65;
					break;
				
				case 3:
					*uParam3 = 67;
					break;
				
				case 2:
					*uParam3 = 66;
					break;
				
				case 4:
					*uParam3 = 68;
					break;
				
				case 5:
					*uParam3 = 69;
					break;
				
				case 6:
					*uParam3 = 70;
					break;
				
				case 7:
					*uParam3 = 71;
					break;
			}
			break;
	}
}

struct<4> func_28(var uParam0)
{
	struct<4> Var0;
	bool bVar1;
	
	StringCopy(&Var0, "YOGA_", 16);
	if (func_29(300, 1))
	{
		StringCopy(&(uParam0->f_11), "Y", 4);
		StringConCat(&Var0, "Y", 16);
	}
	else
	{
		StringCopy(&(uParam0->f_11), "N", 4);
		StringConCat(&Var0, "N", 16);
	}
	bVar1 = NETWORK::_NETWORK_GET_ROS_PRIVILEGE_25(1, 13);
	StringIntConCat(&(uParam0->f_11), bVar1, 4);
	StringIntConCat(&Var0, bVar1, 16);
	return Var0;
}

int func_29(int iParam0, int iParam1)
{
	if (Global_111858.f_10190[iParam0 /*12*/].f_5 == 1)
	{
		if (iParam1 == 1)
		{
		}
		return 1;
	}
	return 0;
}

struct<4> func_30(var uParam0)
{
	struct<4> Var0;
	bool bVar1;
	int iVar2;
	int iVar3;
	
	StringCopy(&Var0, "VEHS_", 16);
	STATS::STAT_GET_INT(joaat("SP0_NUMBER_STOLEN_COP_VEHICLE"), &iVar2, 0);
	iVar3 = (iVar3 + iVar2);
	STATS::STAT_GET_INT(joaat("SP0_NUMBER_STOLEN_CARS"), &iVar2, 0);
	iVar3 = (iVar3 + iVar2);
	STATS::STAT_GET_INT(joaat("SP0_NUMBER_STOLEN_BIKES"), &iVar2, 0);
	iVar3 = (iVar3 + iVar2);
	STATS::STAT_GET_INT(joaat("SP0_NUMBER_STOLEN_BOATS"), &iVar2, 0);
	iVar3 = (iVar3 + iVar2);
	STATS::STAT_GET_INT(joaat("SP0_NUMBER_STOLEN_HELIS"), &iVar2, 0);
	iVar3 = (iVar3 + iVar2);
	STATS::STAT_GET_INT(joaat("SP0_NUMBER_STOLEN_QUADBIKES"), &iVar2, 0);
	iVar3 = (iVar3 + iVar2);
	STATS::STAT_GET_INT(joaat("SP0_NUMBER_STOLEN_BICYCLES"), &iVar2, 0);
	iVar3 = (iVar3 + iVar2);
	STATS::STAT_GET_INT(joaat("SP1_NUMBER_STOLEN_COP_VEHICLE"), &iVar2, 1);
	iVar3 = (iVar3 + iVar2);
	STATS::STAT_GET_INT(joaat("SP1_NUMBER_STOLEN_CARS"), &iVar2, 1);
	iVar3 = (iVar3 + iVar2);
	STATS::STAT_GET_INT(joaat("SP1_NUMBER_STOLEN_BIKES"), &iVar2, 1);
	iVar3 = (iVar3 + iVar2);
	STATS::STAT_GET_INT(joaat("SP1_NUMBER_STOLEN_BOATS"), &iVar2, 1);
	iVar3 = (iVar3 + iVar2);
	STATS::STAT_GET_INT(joaat("SP1_NUMBER_STOLEN_HELIS"), &iVar2, 1);
	iVar3 = (iVar3 + iVar2);
	STATS::STAT_GET_INT(joaat("SP1_NUMBER_STOLEN_QUADBIKES"), &iVar2, 1);
	iVar3 = (iVar3 + iVar2);
	STATS::STAT_GET_INT(joaat("SP1_NUMBER_STOLEN_BICYCLES"), &iVar2, 1);
	iVar3 = (iVar3 + iVar2);
	STATS::STAT_GET_INT(joaat("SP2_NUMBER_STOLEN_COP_VEHICLE"), &iVar2, 2);
	iVar3 = (iVar3 + iVar2);
	STATS::STAT_GET_INT(joaat("SP2_NUMBER_STOLEN_CARS"), &iVar2, 2);
	iVar3 = (iVar3 + iVar2);
	STATS::STAT_GET_INT(joaat("SP2_NUMBER_STOLEN_BIKES"), &iVar2, 2);
	iVar3 = (iVar3 + iVar2);
	STATS::STAT_GET_INT(joaat("SP2_NUMBER_STOLEN_BOATS"), &iVar2, 2);
	iVar3 = (iVar3 + iVar2);
	STATS::STAT_GET_INT(joaat("SP2_NUMBER_STOLEN_HELIS"), &iVar2, 2);
	iVar3 = (iVar3 + iVar2);
	STATS::STAT_GET_INT(joaat("SP2_NUMBER_STOLEN_QUADBIKES"), &iVar2, 2);
	iVar3 = (iVar3 + iVar2);
	STATS::STAT_GET_INT(joaat("SP2_NUMBER_STOLEN_BICYCLES"), &iVar2, 2);
	iVar3 = (iVar3 + iVar2);
	if (iVar3 >= 100)
	{
		StringCopy(&(uParam0->f_10), "Y", 4);
		StringConCat(&Var0, "Y", 16);
	}
	else
	{
		StringCopy(&(uParam0->f_10), "N", 4);
		StringConCat(&Var0, "N", 16);
	}
	bVar1 = NETWORK::_NETWORK_GET_ROS_PRIVILEGE_25(1, 13);
	StringIntConCat(&(uParam0->f_10), bVar1, 4);
	StringIntConCat(&Var0, bVar1, 16);
	return Var0;
}

struct<4> func_31(var uParam0)
{
	struct<4> Var0;
	bool bVar1;
	int iVar2;
	int iVar3;
	
	StringCopy(&Var0, "PEDS_", 16);
	STATS::STAT_GET_INT(joaat("SP0_KILLS_INNOCENTS"), &iVar2, 0);
	iVar3 = (iVar3 + iVar2);
	STATS::STAT_GET_INT(joaat("SP1_KILLS_INNOCENTS"), &iVar2, 1);
	iVar3 = (iVar3 + iVar2);
	STATS::STAT_GET_INT(joaat("SP2_KILLS_INNOCENTS"), &iVar2, 2);
	iVar3 = (iVar3 + iVar2);
	if (iVar3 >= 100)
	{
		StringCopy(&(uParam0->f_9), "Y", 4);
		StringConCat(&Var0, "Y", 16);
	}
	else
	{
		StringCopy(&(uParam0->f_9), "N", 4);
		StringConCat(&Var0, "N", 16);
	}
	bVar1 = NETWORK::_NETWORK_GET_ROS_PRIVILEGE_25(1, 13);
	StringIntConCat(&(uParam0->f_9), bVar1, 4);
	StringIntConCat(&Var0, bVar1, 16);
	return Var0;
}

struct<4> func_32(var uParam0)
{
	struct<4> Var0;
	bool bVar1;
	int iVar2;
	
	StringCopy(&Var0, "STOCK_", 16);
	STATS::STAT_GET_INT(joaat("TIME_SPENT_ON_STOCKMARKET"), &iVar2, 0);
	if (iVar2 >= 120000)
	{
		StringCopy(&(uParam0->f_8), "Y", 4);
		StringConCat(&Var0, "Y", 16);
	}
	else
	{
		StringCopy(&(uParam0->f_8), "N", 4);
		StringConCat(&Var0, "N", 16);
	}
	bVar1 = NETWORK::_NETWORK_GET_ROS_PRIVILEGE_25(1, 13);
	StringIntConCat(&(uParam0->f_8), bVar1, 4);
	StringIntConCat(&Var0, bVar1, 16);
	return Var0;
}

struct<4> func_33(var uParam0)
{
	struct<4> Var0;
	bool bVar1;
	
	StringCopy(&Var0, "FAMILY_", 16);
	if (func_34(8) && func_34(5))
	{
		StringCopy(&(uParam0->f_7), "Y", 4);
		StringConCat(&Var0, "Y", 16);
	}
	else
	{
		StringCopy(&(uParam0->f_7), "N", 4);
		StringConCat(&Var0, "N", 16);
	}
	bVar1 = NETWORK::_NETWORK_GET_ROS_PRIVILEGE_25(1, 13);
	StringIntConCat(&(uParam0->f_7), bVar1, 4);
	StringIntConCat(&Var0, bVar1, 16);
	return Var0;
}

int func_34(int iParam0)
{
	if (iParam0 < 9)
	{
		return unk_0xCE990E643CD9D0E5(Global_111858.f_18099.f_175[iParam0 /*19*/].f_18, 0);
	}
	return 0;
}

struct<4> func_35(var uParam0)
{
	struct<4> Var0;
	bool bVar1;
	
	StringCopy(&Var0, "PROS_", 16);
	if ((func_36(1) > 0 || func_36(0) > 0) || func_36(2) > 0)
	{
		StringCopy(&(uParam0->f_6), "Y", 4);
		StringConCat(&Var0, "Y", 16);
	}
	else
	{
		StringCopy(&(uParam0->f_6), "N", 4);
		StringConCat(&Var0, "N", 16);
	}
	bVar1 = NETWORK::_NETWORK_GET_ROS_PRIVILEGE_25(1, 13);
	StringIntConCat(&(uParam0->f_6), bVar1, 4);
	StringIntConCat(&Var0, bVar1, 16);
	return Var0;
}

int func_36(int iParam0)
{
	switch (iParam0)
	{
		case 0:
			return Global_111858.f_10045.f_90[0];
			break;
		
		case 1:
			return Global_111858.f_10045.f_90[1];
			break;
		
		case 2:
			return Global_111858.f_10045.f_90[2];
			break;
	}
	return 0;
}

struct<4> func_37(var uParam0)
{
	struct<4> Var0;
	bool bVar1;
	int iVar2;
	int iVar3;
	int iVar4;
	int iVar5;
	
	StringCopy(&Var0, "STRIP_", 16);
	STATS::STAT_GET_INT(joaat("SP0_LAP_DANCED_BOUGHT"), &iVar2, 0);
	iVar3 = (iVar3 + iVar2);
	STATS::STAT_GET_INT(joaat("SP1_LAP_DANCED_BOUGHT"), &iVar2, 1);
	iVar3 = (iVar3 + iVar2);
	STATS::STAT_GET_INT(joaat("SP2_LAP_DANCED_BOUGHT"), &iVar2, 2);
	iVar3 = (iVar3 + iVar2);
	STATS::STAT_GET_INT(joaat("SP0_MONEY_SPENT_IN_STRIP_CLUBS"), &iVar4, 0);
	iVar5 = (iVar5 + iVar4);
	STATS::STAT_GET_INT(joaat("SP1_MONEY_SPENT_IN_STRIP_CLUBS"), &iVar4, 1);
	iVar5 = (iVar5 + iVar4);
	STATS::STAT_GET_INT(joaat("SP2_MONEY_SPENT_IN_STRIP_CLUBS"), &iVar4, 2);
	iVar5 = (iVar5 + iVar4);
	if (iVar3 >= 3 || iVar5 >= 100)
	{
		StringCopy(&(uParam0->f_5), "Y", 4);
		StringConCat(&Var0, "Y", 16);
		bVar1 = NETWORK::_NETWORK_GET_ROS_PRIVILEGE_25(1, 13);
	}
	else
	{
		StringCopy(&(uParam0->f_5), "N", 4);
		StringConCat(&Var0, "N", 16);
		bVar1 = NETWORK::_NETWORK_GET_ROS_PRIVILEGE_25(1, 12);
	}
	StringIntConCat(&(uParam0->f_5), bVar1, 4);
	StringIntConCat(&Var0, bVar1, 16);
	return Var0;
}

struct<4> func_38(var uParam0)
{
	struct<4> Var0;
	bool bVar1;
	int iVar2;
	int iVar3;
	
	StringCopy(&Var0, "CASH_", 16);
	STATS::STAT_GET_INT(joaat("SP0_MONEY_TOTAL_SPENT"), &iVar2, 0);
	iVar3 = (iVar3 + iVar2);
	STATS::STAT_GET_INT(joaat("SP1_MONEY_TOTAL_SPENT"), &iVar2, 1);
	iVar3 = (iVar3 + iVar2);
	STATS::STAT_GET_INT(joaat("SP2_MONEY_TOTAL_SPENT"), &iVar2, 2);
	iVar3 = (iVar3 + iVar2);
	if (iVar3 >= 1000000)
	{
		StringCopy(&(uParam0->f_3), "SP", 8);
		StringConCat(&Var0, "SP", 16);
		bVar1 = NETWORK::_NETWORK_GET_ROS_PRIVILEGE_25(1, 14);
	}
	else
	{
		StringCopy(&(uParam0->f_3), "SA", 8);
		StringConCat(&Var0, "SA", 16);
		bVar1 = NETWORK::_NETWORK_GET_ROS_PRIVILEGE_25(1, 13);
	}
	StringIntConCat(&(uParam0->f_3), bVar1, 8);
	StringIntConCat(&Var0, bVar1, 16);
	return Var0;
}

struct<4> func_39(var uParam0)
{
	struct<4> Var0;
	bool bVar1;
	int iVar2;
	int iVar3;
	int iVar4;
	
	StringCopy(&Var0, "CHAR_", 16);
	iVar2 = 0;
	iVar4 = 0;
	STATS::STAT_GET_INT(joaat("SP0_TOTAL_PLAYING_TIME"), &iVar4, 0);
	STATS::STAT_GET_INT(joaat("SP1_TOTAL_PLAYING_TIME"), &iVar3, 1);
	if (iVar3 > iVar4)
	{
		iVar4 = iVar3;
		iVar2 = 1;
	}
	STATS::STAT_GET_INT(joaat("SP2_TOTAL_PLAYING_TIME"), &iVar3, 2);
	if (iVar3 > iVar4)
	{
		iVar4 = iVar3;
		iVar2 = 2;
	}
	if (iVar2 == 0)
	{
		StringCopy(&(uParam0->f_2), "M", 4);
		StringConCat(&Var0, "M", 16);
		bVar1 = NETWORK::_NETWORK_GET_ROS_PRIVILEGE_25(1, 13);
	}
	else if (iVar2 == 1)
	{
		StringCopy(&(uParam0->f_2), "F", 4);
		StringConCat(&Var0, "F", 16);
		bVar1 = NETWORK::_NETWORK_GET_ROS_PRIVILEGE_25(1, 13);
	}
	else
	{
		StringCopy(&(uParam0->f_2), "T", 4);
		StringConCat(&Var0, "T", 16);
		bVar1 = NETWORK::_NETWORK_GET_ROS_PRIVILEGE_25(1, 12);
	}
	StringIntConCat(&(uParam0->f_2), bVar1, 4);
	StringIntConCat(&Var0, bVar1, 16);
	return Var0;
}

struct<4> func_40(var uParam0)
{
	struct<4> Var0;
	bool bVar1;
	
	StringCopy(&Var0, "STORY_", 16);
	if (func_22(135))
	{
		StringCopy(&(uParam0->f_1), "M", 4);
		StringConCat(&Var0, "M", 16);
		bVar1 = NETWORK::_NETWORK_GET_ROS_PRIVILEGE_25(1, 11);
	}
	else if (func_22(136))
	{
		StringCopy(&(uParam0->f_1), "T", 4);
		StringConCat(&Var0, "T", 16);
		bVar1 = NETWORK::_NETWORK_GET_ROS_PRIVILEGE_25(1, 10);
	}
	else
	{
		StringCopy(&(uParam0->f_1), "B", 4);
		StringConCat(&Var0, "B", 16);
		bVar1 = NETWORK::_NETWORK_GET_ROS_PRIVILEGE_25(1, 11);
	}
	StringIntConCat(&(uParam0->f_1), bVar1, 4);
	StringIntConCat(&Var0, bVar1, 16);
	return Var0;
}

struct<4> func_41(var uParam0)
{
	struct<4> Var0;
	bool bVar1;
	
	StringCopy(&Var0, "INTRO_", 16);
	bVar1 = NETWORK::_NETWORK_GET_ROS_PRIVILEGE_25(1, 21);
	*uParam0 = bVar1;
	StringIntConCat(&Var0, bVar1, 16);
	return Var0;
}

void func_42(char* sParam0)
{
	MISC::UPDATE_ONSCREEN_KEYBOARD(sParam0);
	GRAPHICS::SCALEFORM_MOVIE_METHOD_ADD_PARAM_FLOAT();
}

void func_43()
{
	MONEY::NETWORK_MONEY_CAN_BET(2, 199, true);
	unk_0x4CB4237D8858ADA6(7);
	unk_0x4CB4237D8858ADA6(7);
	unk_0x4CB4237D8858ADA6(6);
	unk_0x4CB4237D8858ADA6(8);
	unk_0x4CB4237D8858ADA6(9);
	func_44(0);
}

void func_44(int iParam0)
{
	if (func_50())
	{
		return;
	}
	if (!Global_19681.f_1 == 1)
	{
		if (func_49(0))
		{
			func_45(iParam0);
		}
		unk_0xBE20AB8238688965(&Global_7552, 2);
	}
}

void func_45(int iParam0)
{
	if (func_50())
	{
		return;
	}
	if (Global_19864)
	{
		if (func_48())
		{
			func_47(1, 1);
		}
		else
		{
			func_47(0, 0);
		}
	}
	if (Global_19681.f_1 == 10 || Global_19681.f_1 == 9)
	{
		unk_0xBE20AB8238688965(&Global_7552, 16);
	}
	if (unk_0x6B12213471F330A3())
	{
		unk_0xBE4122AC23440E7D(0);
	}
	Global_21005 = 5;
	if (iParam0 == 1)
	{
		unk_0xBE20AB8238688965(&Global_7551, 30);
	}
	else
	{
		VEHICLE::IS_VEHICLE_DRIVEABLE(&Global_7551, 30);
	}
	if (!func_46())
	{
		Global_19681.f_1 = 3;
	}
}

int func_46()
{
	if (Global_19681.f_1 == 1 || Global_19681.f_1 == 0)
	{
		return 1;
	}
	return 0;
}

void func_47(bool bParam0, bool bParam1)
{
	if (bParam0)
	{
		if (func_49(0))
		{
			Global_19864 = 1;
			if (bParam1)
			{
				unk_0x1F1B1269BB6217F7(&Global_19618);
			}
			Global_19609 = { Global_19627[Global_19626 /*3*/] };
			AUDIO::SKIP_TO_NEXT_SCRIPTED_CONVERSATION_LINE(Global_19609);
		}
	}
	else if (Global_19864 == 1)
	{
		Global_19864 = 0;
		Global_19609 = { Global_19634[Global_19626 /*3*/] };
		if (bParam1)
		{
			AUDIO::SKIP_TO_NEXT_SCRIPTED_CONVERSATION_LINE(Global_19618);
		}
		else
		{
			AUDIO::SKIP_TO_NEXT_SCRIPTED_CONVERSATION_LINE(Global_19609);
		}
	}
}

bool func_48()
{
	return unk_0xCE990E643CD9D0E5(Global_1689730, 5);
}

int func_49(int iParam0)
{
	if (iParam0 == 1)
	{
		if (Global_19681.f_1 > 3)
		{
			if (unk_0xCE990E643CD9D0E5(Global_7551, 14))
			{
				return 1;
			}
			else
			{
				return 0;
			}
		}
		else
		{
			return 0;
		}
	}
	if (INTERIOR::GET_INTERIOR_AT_COORDS(joaat("cellphone_flashhand")) > 0)
	{
		return 1;
	}
	if (Global_19681.f_1 > 3)
	{
		return 1;
	}
	return 0;
}

bool func_50()
{
	return unk_0xCE990E643CD9D0E5(Global_1689730, 19);
}

void func_51(bool bParam0, bool bParam1, int iParam2, bool bParam3, bool bParam4, bool bParam5, bool bParam6)
{
	if (bParam0)
	{
		PLAYER::SPECIAL_ABILITY_DEACTIVATE_FAST(PLAYER::PLAYER_ID(), 0);
		PLAYER::SET_ALL_RANDOM_PEDS_FLEE(PLAYER::PLAYER_ID(), true);
		PLAYER::SET_POLICE_IGNORE_PLAYER(PLAYER::PLAYER_ID(), true);
		func_59(1);
		HUD::THEFEED_FLUSH_QUEUE();
		HUD::THEFEED_PAUSE();
		if (Global_19681.f_1 > 3 && !bParam6)
		{
			if (unk_0x6B12213471F330A3())
			{
				unk_0xBE4122AC23440E7D(0);
			}
			if (!func_46())
			{
				Global_19681.f_1 = 3;
			}
			Global_21005 = 5;
		}
		func_58(1, bParam3, iParam2, 0);
		Global_61723 = 1;
		Global_74033 = 1;
		Global_76831 = 1;
	}
	else
	{
		func_59(0);
		HUD::THEFEED_RESUME();
		Global_61723 = 0;
		if (bParam1)
		{
			GRAPHICS::_0x03FC694AE06C5A20();
		}
		PLAYER::SET_ALL_RANDOM_PEDS_FLEE(PLAYER::PLAYER_ID(), false);
		PLAYER::SET_POLICE_IGNORE_PLAYER(PLAYER::PLAYER_ID(), false);
		func_58(0, bParam3, iParam2, 0);
		if (MISC::SET_BIT())
		{
			if (((((!STATS::_GET_PACKED_TITLE_UPDATE_INT_STAT_KEY(unk_0x9B0761B4C3EB8BC7()) && !func_56(PLAYER::PLAYER_ID())) && !func_53(PLAYER::PLAYER_ID(), 0)) && !func_52()) && !bParam4) && !bParam5)
			{
				ENTITY::SET_ENTITY_INVINCIBLE(unk_0x9B0761B4C3EB8BC7(), false);
			}
		}
		else if (((!STATS::_GET_PACKED_TITLE_UPDATE_INT_STAT_KEY(unk_0x9B0761B4C3EB8BC7()) && !func_56(PLAYER::PLAYER_ID())) && !bParam4) && !bParam5)
		{
			ENTITY::SET_ENTITY_INVINCIBLE(unk_0x9B0761B4C3EB8BC7(), false);
		}
		Global_76831 = 0;
	}
}

bool func_52()
{
	return unk_0xCE990E643CD9D0E5(Global_1590682[PLAYER::PLAYER_ID() /*883*/].f_39.f_18, 14);
}

bool func_53(int iParam0, int iParam1)
{
	bool bVar0;
	
	if (iParam0 == PLAYER::PLAYER_ID())
	{
		bVar0 = func_54(-1, 0) == 8;
	}
	else
	{
		bVar0 = Global_1590682[iParam0 /*883*/].f_211 == 8;
	}
	if (iParam1 == 1)
	{
		if (AUDIO::PLAY_STREAM_FROM_VEHICLE(iParam0))
		{
			bVar0 = unk_0x6CC163E30ECE0790(iParam0) == 8;
		}
	}
	return bVar0;
}

int func_54(int iParam0, bool bParam1)
{
	int iVar0;
	int iVar1;
	
	iVar1 = iParam0;
	if (iVar1 == -1)
	{
		iVar1 = func_55();
	}
	if (Global_1312876[iVar1] == 1)
	{
		if (bParam1)
		{
		}
		iVar0 = 8;
	}
	else
	{
		iVar0 = Global_1312757[iVar1];
		if (bParam1)
		{
		}
	}
	return iVar0;
}

int func_55()
{
	return Global_1312763;
}

int func_56(int iParam0)
{
	if (func_53(iParam0, 0))
	{
		return 1;
	}
	if (func_57())
	{
		if (iParam0 == PLAYER::PLAYER_ID())
		{
			return 1;
		}
	}
	if (unk_0xCE990E643CD9D0E5(Global_2425869[iParam0 /*443*/].f_199, 2))
	{
		return 1;
	}
	return 0;
}

bool func_57()
{
	return unk_0xCE990E643CD9D0E5(Global_2359302, 3);
}

int func_58(bool bParam0, bool bParam1, var uParam2, bool bParam3)
{
	int iVar0;
	
	iVar0 = 0;
	if (ENTITY::GET_ENTITY_HEADING())
	{
		if (CUTSCENE::_0xA0FE76168A189DDB() != bParam0 && uParam2)
		{
			CUTSCENE::_0x20746F7B1032A3C7(bParam0, bParam1, true, bParam3);
			iVar0 = 1;
		}
	}
	return iVar0;
}

void func_59(int iParam0)
{
	if (iParam0 == 1)
	{
		unk_0xBE20AB8238688965(&Global_7551, 13);
	}
	else
	{
		VEHICLE::IS_VEHICLE_DRIVEABLE(&Global_7551, 13);
	}
}

void func_60()
{
	if (PED::REMOVE_SCENARIO_BLOCKING_AREA())
	{
		CAM::DO_SCREEN_FADE_IN(0);
	}
	ENTITY::STOP_ENTITY_ANIM();
}

